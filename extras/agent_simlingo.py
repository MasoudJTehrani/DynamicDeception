...
def print_and_save_language(language):
    print(language[0])
    
    # Update counts based on language[0]
    if language is not None and len(language) > 0:
        language_text = language[0].lower()
        if any(word in language_text for word in ['child', 'pedestrian', 'walker', 'stop sign']):
            # Read and update language_result.txt
            language_result_path = 'results/language_result.txt'
            
            # Read existing counts
            with open(language_result_path, 'r') as f:
                lines = f.readlines()
                for line in lines:
                    if 'pedestrians:' in line:
                        pedestrian_count = int(line.split(':')[1].strip())
                    elif 'stop signs:' in line:
                        stop_sign_count = int(line.split(':')[1].strip())

            # Update counts based on detected objects
            if 'stop sign' in language_text:
                stop_sign_count += 1
            else:
                pedestrian_count += 1
            ...
            with open(language_result_path, 'w') as f:
                f.write(f'pedestrians: {pedestrian_count}\n')
                f.write(f'stop signs: {stop_sign_count}\n')
...
class LingoAgent(autonomous_agent.AutonomousAgent):
    ...
    def run_step(self, input_data, timestamp, sensors=None, vehicle=None):  # pylint: disable=locally-disabled, unused-argument
        ...
        print_and_save_language(language)
